...
if __name__ == '__main__':
    ...
    word_dict = loader_v2.word_dict()
    dict_dim = len(word_dict)
    class_dim = 2
    train_reader = paddle.batch(
        paddle.reader.shuffle(
            lambda: loader_v2.train(word_dict), buf_size=1000),
        batch_size=100)
    test_reader = paddle.batch(
        lambda: loader_v2.test(word_dict), batch_size=100)
    ...
    def event_handler(event):
        ...
        if isinstance(event, paddle.event.EndPass):
            with open('./params_pass2_%d.tar' % event.pass_id, 'w') as f:
                parameters.to_tar(f)
    ...
    with open("./inference_topology2.pkl", 'wb') as f:
